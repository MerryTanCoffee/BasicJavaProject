	...
	<div class="row">
		<div class="col-xs-12">
			...
			<table style="width:80%; margin-top:20px">
				<tr class="portlet-title" style="height:80px;">
				<td style="height:80px;">
					<img alt="톡" src="./img/chat.png" width="60px">
				</td>
				<td style="height:80px; font-size:25px">
					<i class="fa fa-circle text-green"></i>'<%=mv.getMemNick()%>'와 '<%=mv.getReceiverNick()%>'의 채팅
				</td>
				</tr>
			</table>
...
<form id="insertFrm" name="insertFrm">
		<div id="myid" style="background-color: #dce5ed; font-size:25px;">
		...
		<%if(mv.getMemId().equals(chatList.get(i).getFromId())) { %>
			<br>
			<div style="text-align: right; font-size:15px;"><%=chatList.get(i).getFromNick()%></div>
			<div style="text-align: right"><%=chatList.get(i).getChatContent()%></div>
			<%} else{ %>
			<br>
			<div style="font-size:15px;"><%=chatList.get(i).getFromNick()%></div>
...
<!-- 				<div id="chatList" class="porlet-body chat-widget" style="overflow-y: auto; width: auto; height: 500px; "> -->
<!-- 				</div> -->
			...
			<textarea onkeyup="if(window.event.keyCode==13){myfunction()}" name="chatContent" style="height: 80px;" id="chatContent" class="form-control" placeholder="내용 입력"></textarea>
					...
					<button type="button" class="btn btn-default pull-right" id="insertBtn" style="width:100px; height:40px">보내기</button>
					</div>
					</div>
				</div>
			</div>
