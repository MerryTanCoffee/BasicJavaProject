...
public class DetailAskController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		int askNo = Integer.parseInt(askNos);
		...
		List<AComVO> acv = askcomService.getAskcomList(askNo);
		
		req.setAttribute("av", av);
		req.setAttribute("askNo", askNo);
		req.setAttribute("acv", acv);
		
		req.getRequestDispatcher("/ask/detailAsk.jsp").forward(req, resp);
	}
@Override
